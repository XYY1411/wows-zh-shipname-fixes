...
`scripts/build_release.py` 为每个版本（standard / full）同时输出**完整版**与**增量版**，按 zh（国服简中）/ zh_cn（zh 拷贝，兼容不同客户端命名）/ zh_sg（国际服简中）分别提供：
...
│   ├── zh/wowsZhShipnameFixes.mo           # 增量版(国服, loader 用)
│   ├── zh_cn/LC_MESSAGES/global.mo         # 完整版(zh 拷贝, 兼容 zh_cn 命名)
│   ├── zh_cn/wowsZhShipnameFixes.mo        # 增量版(与 zh 同)
│   ├── zh_sg/LC_MESSAGES/global.mo         # 完整版(国际服简中, 无 loader 用)
│   ├── zh_sg/wowsZhShipnameFixes.mo        # 增量版(国际服, loader 用)
│   └── version.txt
└── full/                     # 全名版(缩写键用 _FULL, 结构同 standard)
```

- **无 loader**：取 `standard/`（或 `full/`）的 `zh/`（`zh_cn/` `/` `zh_sg/`）`LC_MESSAGES/global.mo` 放进 `res_mods/texts/<语言>/LC_MESSAGES/`
- **有 loader（阿斯兰）**：取 `wowsZhShipnameFixes.mo` 放进 `res_mods/texts/<语言>/`（loader 递归读取），由 loader 加载

## 机制

- **完整版**：官方该语言键集合 + 舰船键覆盖为最终翻译（`full` 版先把缩写键换为 `_FULL` 值）；用自定义编译器遍历全部条目（含空翻译键，避免显示键值）
- **增量版**：只保留「最终翻译 ≠ 官方 zh/zh_sg 任一」的舰船键，`Priority: 60`；直接放语言目录下，同一份复制到 zh / zh_cn / zh_sg
...
内部 `res_mods/texts/<zh|zh_cn|zh_sg>/LC_MESSAGES/<对应 mo>`；也可用手动工作流发布为 Release。
